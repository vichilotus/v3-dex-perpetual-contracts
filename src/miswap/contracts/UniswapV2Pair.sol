...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    function getPredicts(bool _switch) public view override returns (uint256 _price0Predict, uint256 _price1Predict) {
        if (_switch) {
    ...
    function updateFee(bool _feeOn, uint256 feeAmount) external override lock {
        ...
        if (_feeOn) {
            ...
            _update(balance0, balance1, uint112(_reserve0), uint112(_reserve1), false);
        }
    }

    function updatePredict(uint256 _predict0, uint256 _predict1) external override lock {
        address feeTo = IUniswapV2Factory(factory).feeToSetter();
        require(msg.sender == feeTo, "UniswapV2: FORBIDDEN");
    ...
    function _update(uint256 balance0, uint256 balance1, uint112 _reserve0, uint112 _reserve1, bool checkedReserve) private {
        if (checkedReserve) {
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external override lock {
        ...
        uint256 balance0 = IERC20(token0).balanceOf(address(this));
        uint256 balance1 = IERC20(token1).balanceOf(address(this));
        require(price0Predict < balance0 - amount0Out && price1Predict < balance1 - amount1Out, "UniswapV2: INSUFFICIENT_PREDICT_AMOUNT");
        {
    ...
    function sync() external override lock {
        _update(IERC20(token0).balanceOf(address(this)), IERC20(token1).balanceOf(address(this)), reserve0, reserve1, true);
    }
}
