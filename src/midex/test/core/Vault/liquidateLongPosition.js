...
describe('Vault.liquidateLongPosition', function () {
    ...
    it('liquidate long', async () => {
        ...
        await expect(vaultPositionController.connect(user0).liquidatePosition(user0.address, btc.address, btc.address, true, user2.address)).to.be.revertedWith(
        ...
        await increaseBlockTime(provider, 10)
        await fulfillController.requestUpdatePrices()
        await miOracle.fulfillRequest([{ tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(43500), lastUpdate: 0 }], 0)
        ...
        await expect(vaultPositionController.liquidatePosition(user0.address, btc.address, btc.address, true, user2.address)).to.be.revertedWith(
            'Vault: position cannot be liquidated'
        )

        await increaseBlockTime(provider, 10)
        await fulfillController.requestUpdatePrices()
        await miOracle.fulfillRequest([{ tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(38700), lastUpdate: 0 }], 0)
        delta = await vaultPositionController.getPositionDelta(user0.address, btc.address, btc.address, true)
        expect(delta[0]).eq(false)
        ...
        await expect(vaultPositionController.connect(user1).liquidatePosition(user0.address, btc.address, btc.address, true, user2.address)).to.be.revertedWith(
        ...
        expect(await milpManager.getAumInMusd(false)).eq('99064997000000000000') // 99.064997
        expect(await milpManager.getAumInMusd(true)).eq('101418485000000000000') // 101.418485
        ...
        expect(await milpManager.getAumInMusd(false)).eq('101522097000000000000') // 101.522097
        expect(await milpManager.getAumInMusd(true)).eq('114113985000000000000') // 114.113985
    ...
    it('automatic stop-loss', async () => {
        ...
        await increaseBlockTime(provider, 10)
        await fulfillController.requestUpdatePrices()
        await miOracle.fulfillRequest([{ tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(37760), lastUpdate: 0 }], 0)
