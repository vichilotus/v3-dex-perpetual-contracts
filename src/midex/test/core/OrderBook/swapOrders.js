...
const { getDefault, validateOrderFields, getTxFees, getMinOut, defaultCreateSwapOrderFactory, getTriggerRatio } = require('./helpers')
...
describe('OrderBook, swap orders', function () {
    ...
    beforeEach(async () => {
        ...
        await miOracle.fulfillRequest(
            [
                ...
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(BTC_PRICE), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BNB, price: toMiOraclePrice(BNB_PRICE), lastUpdate: 0 },
            ],
            0
        )
        ...
        // it's impossible to just mint musd (?)
        await router.connect(user0).swap([dai.address, musd.address], expandDecimals(10000, 18), expandDecimals(9900, 18), user0.address)
        await miOracle.fulfillRequest(
            [
                { tokenIndex: tokenIndexes.USDT, price: toMiOraclePrice(1), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(BTC_PRICE), lastUpdate: 0 },
        ...
        await musd.connect(user0).approve(router.address, expandDecimals(9900, 18))
    ...
    [x] if conditions are met executor receives fee
    ...
    */
    ...
    it('cancelSwapOrder, tokenA != BNB', async () => {
        ...
        reportGasUsed(provider, tx, 'cancelSwapOrder')
    ...
    it('updateSwapOrder', async () => {
        ...
        await expect(orderBook.connect(user1).updateSwapOrder(0, newMinOut, newTriggerRatio, newTriggerAboveThreshold)).to.be.revertedWith(
            'OrderBook: non-existent order'
        )

        await expect(orderBook.connect(defaults.user).updateSwapOrder(1, newMinOut, newTriggerRatio, newTriggerAboveThreshold)).to.be.revertedWith(
            'OrderBook: non-existent order'
        )
    ...
    it('executeSwapOrder, triggerAboveThreshold == false', async () => {
        ...
        const minOut = await getMinOut(tokenDecimals, getTriggerRatio(toUsd(BTC_PRICE), toUsd(BNB_PRICE - 50)), path, amountIn)
        ...
        await expect(orderBook.executeSwapOrder(defaults.user.address, 2, user1.address), 'non-existent order').to.be.revertedWith(
            'OrderBook: non-existent order'
        )
        ...
        await miOracle.fulfillRequest(
            [
                { tokenIndex: tokenIndexes.USDT, price: toMiOraclePrice(1), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(BTC_PRICE), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BNB, price: toMiOraclePrice(BNB_PRICE - 30), lastUpdate: 0 },
            ],
            0
        )

        await expect(orderBook.executeSwapOrder(defaults.user.address, 0, user1.address), 'insufficient amountOut').to.be.revertedWith(
        ...
        await miOracle.fulfillRequest(
            [
                { tokenIndex: tokenIndexes.USDT, price: toMiOraclePrice(1), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(BTC_PRICE), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BNB, price: toMiOraclePrice(BNB_PRICE - 70), lastUpdate: 0 },
            ],
            0
        )
    ...
    it('executeSwapOrder, triggerAboveThreshold == false, DAI -> WBNB, shouldUnwrap = false', async () => {
        ...
        const minOut = await getMinOut(tokenDecimals, getTriggerRatio(toUsd(1), toUsd(BNB_PRICE + 50)), path, amountIn)
    ...
    it('executeSwapOrder, triggerAboveThreshold == true', async () => {
        ...
        const minOut = await getMinOut(tokenDecimals, getTriggerRatio(toUsd(BNB_PRICE), toUsd(63000)), path, amountIn)
        ...
        await expect(orderBook.executeSwapOrder(defaults.user.address, 2, executor.address)).to.be.revertedWith('OrderBook: non-existent order')
        ...
        await miOracle.fulfillRequest(
            [
                { tokenIndex: tokenIndexes.USDT, price: toMiOraclePrice(1), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(60500), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BNB, price: toMiOraclePrice(BNB_PRICE), lastUpdate: 0 },
            ],
            0
        )

        await expect(orderBook.executeSwapOrder(defaults.user.address, 0, executor.address)).to.be.revertedWith('OrderBook: invalid price for execution')
        ...
        await miOracle.fulfillRequest(
            [
                { tokenIndex: tokenIndexes.USDT, price: toMiOraclePrice(1), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(62500), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BNB, price: toMiOraclePrice(BNB_PRICE), lastUpdate: 0 },
            ],
            0
        )
    ...
    it('executeSwapOrder, triggerAboveThreshold == true, MUSD -> BTC', async () => {
        const triggerRatio = getTriggerRatio(toUsd(1), toUsd(62000))
        const amountIn = expandDecimals(1000, 18)
        const path = [musd.address, btc.address]
        const value = defaults.executionFee

        // minOut is not mandatory for such orders but with minOut it's possible to limit max price
        // e.g. user would not be happy if he sets order "buy if BTC > $65000" and order executes with $75000
        const minOut = await getMinOut(tokenDecimals, getTriggerRatio(toUsd(1), toUsd(63000)), path, amountIn)
        ...
        await miOracle.fulfillRequest(
            [
                { tokenIndex: tokenIndexes.USDT, price: toMiOraclePrice(1), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(70000), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BNB, price: toMiOraclePrice(BNB_PRICE), lastUpdate: 0 },
            ],
            0
        )
        await expect(orderBook.executeSwapOrder(defaults.user.address, 0, executor.address)).to.be.revertedWith('OrderBook: insufficient amountOut')
    ...
    it('executeSwapOrder, triggerAboveThreshold == true, MUSD -> DAI -> BTC', async () => {
        const triggerRatio = getTriggerRatio(toUsd(1), toUsd(62000))
        const amountIn = expandDecimals(1000, 18)
        const path = [musd.address, dai.address, btc.address]
        const value = defaults.executionFee
    ...
    it('executeSwapOrder, triggerAboveThreshold == true, MUSD -> BNB -> BTC', async () => {
        const triggerRatio = getTriggerRatio(toUsd(1), toUsd(62000))
        const amountIn = expandDecimals(1000, 18)
        const path = [musd.address, bnb.address, btc.address]
        const value = defaults.executionFee
    ...
    it('executeSwapOrder, triggerAboveThreshold == true, BTC -> MUSD', async () => {
        const triggerRatio = getTriggerRatio(toUsd(62000), toUsd(1))
        const amountIn = expandDecimals(1, 6) // 0.01 BTC
        const path = [btc.address, musd.address]
        const value = defaults.executionFee

        // minOut is not mandatory for such orders but with minOut it's possible to limit max price
        // e.g. user would not be happy if he sets order "buy if BTC > $65000" and order executes with $75000
        const minOut = await getMinOut(tokenDecimals, getTriggerRatio(toUsd(60000), toUsd(1)), path, amountIn)
        ...
        await miOracle.fulfillRequest(
            [
                { tokenIndex: tokenIndexes.USDT, price: toMiOraclePrice(1), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(63000), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BNB, price: toMiOraclePrice(BNB_PRICE), lastUpdate: 0 },
            ],
            0
        )
        ...
        await miOracle.fulfillRequest(
            [
                { tokenIndex: tokenIndexes.USDT, price: toMiOraclePrice(1), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(50000), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BNB, price: toMiOraclePrice(BNB_PRICE), lastUpdate: 0 },
            ],
            0
        )
        ...
        await miOracle.fulfillRequest(
            [
                { tokenIndex: tokenIndexes.USDT, price: toMiOraclePrice(1), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(61000), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BNB, price: toMiOraclePrice(BNB_PRICE), lastUpdate: 0 },
            ],
            0
        )
        ...
        const executorBalanceBefore = await executor.getBalance()
        const userMusdBalanceBefore = await musd.balanceOf(defaults.user.address)
        ...
        const userMusdBalanceAfter = await musd.balanceOf(defaults.user.address)
        expect(userMusdBalanceAfter.gt(userMusdBalanceBefore.add(minOut)), 'userMusdBalanceAfter').to.be.true
    ...
    it('complex scenario', async () => {
        ...
        await miOracle.fulfillRequest(
            [
                { tokenIndex: tokenIndexes.USDT, price: toMiOraclePrice(1), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(BTC_PRICE + 1500), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BNB, price: toMiOraclePrice(BNB_PRICE), lastUpdate: 0 },
            ],
            0
        )

        await expect(orderBook.executeSwapOrder(defaults.user.address, order1Index, user1.address), 'order1 revert').to.be.revertedWith(
        ...
        await expect(orderBook.executeSwapOrder(defaults.user.address, order2Index, user1.address), 'order2 revert').to.be.revertedWith(
            'OrderBook: insufficient amountOut'
        )
        ...
        await miOracle.fulfillRequest(
            [
                { tokenIndex: tokenIndexes.USDT, price: toMiOraclePrice(1), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(BTC_PRICE + 1500), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BNB, price: toMiOraclePrice(BNB_PRICE + 100), lastUpdate: 0 },
            ],
            0
        )
