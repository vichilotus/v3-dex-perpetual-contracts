...
describe('OrderBook, increase position orders', function () {
    ...
    */
    ...
    it('createIncreaseOrder, bad input', async () => {
        ...
        // orderBook initialize minPurchaseTokenAmountUsd = expandDecimals(5, 30)
        await defaultCreateIncreaseOrder({
            path: [dai.address],
            amountIn: expandDecimals(4, 18), // amountIn < minPurchaseTokenAmountUsd
        }),
            counter++
    ...
    it('updateIncreaseOrder', async () => {
        ...
        await expect(orderBook.connect(user1).updateIncreaseOrder(0, newSizeDelta, newTriggerPrice, newTriggerAboveThreshold)).to.be.revertedWith(
    ...
    it('cancelOrder', async () => {
        ...
        expect(bnbBalanceAfter, 'bnbBalanceAfter').to.be.equal(bnbBalanceBefore.sub(txFees))
    ...
    it('executeOrder, current price is invalid', async () => {
        ...
        for ([triggerPrice, isLong, collateralToken, triggerAboveThreshold, newBtcPrice, setPriceTwice] of [
            ...
            await expect(orderBook.executeIncreaseOrder(order.account, orderIndex, user1.address)).to.be.revertedWith('OrderBook: invalid price for execution')
            ...
            if (setPriceTwice) {
                ...
                await miOracle.fulfillRequest(
                    [
                        { tokenIndex: tokenIndexes.USDT, price: toMiOraclePrice(1), lastUpdate: 0 },
                        { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(newBtcPrice), lastUpdate: 0 },
                        { tokenIndex: tokenIndexes.BNB, price: toMiOraclePrice(BNB_PRICE), lastUpdate: 0 },
                    ],
                    0
                )

                await expect(orderBook.executeIncreaseOrder(order.account, orderIndex, user1.address)).to.be.revertedWith(
    ...
    it('executeOrder, 2 orders with the same position', async () => {
        await defaultCreateIncreaseOrder()
        await miOracle.fulfillRequest(
            [
                { tokenIndex: tokenIndexes.USDT, price: toMiOraclePrice(1), lastUpdate: 0 },
