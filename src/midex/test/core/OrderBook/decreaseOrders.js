...
describe('OrderBook, decrease position orders', () => {
    ...
    */
    ...
    it('updateDecreaseOrder', async () => {
        ...
        await expect(
            orderBook.connect(user1).updateDecreaseOrder(0, newCollateralDelta, newSizeDelta, newTriggerPrice, newTriggerAboveThreshold)
        ).to.be.revertedWith('OrderBook: non-existent order')

        const tx2 = await orderBook.connect(defaults.user).updateDecreaseOrder(0, newCollateralDelta, newSizeDelta, newTriggerPrice, newTriggerAboveThreshold)
    ...
    it('Execute decrease order, invalid price', async () => {
        ...
        for ([triggerPrice, isLong, triggerAboveThreshold, newBtcPrice, setPriceTwice] of [
            ...
            await fulfillController.requestUpdatePrices()
            await miOracle.fulfillRequest([{ tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(BTC_PRICE), lastUpdate: 0 }], 0)
            ...
            await expect(orderBook.executeDecreaseOrder(order.account, orderIndex, user1.address), 1).to.be.revertedWith(
            ...
            if (setPriceTwice) {
                ...
                await increaseBlockTime(provider, 10)
                await fulfillController.requestUpdatePrices()
                await miOracle.fulfillRequest([{ tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(newBtcPrice), lastUpdate: 0 }], 0)
                await expect(orderBook.executeDecreaseOrder(order.account, orderIndex, user1.address), 2).to.be.revertedWith(
                    'OrderBook: invalid price for execution'
                )
            }
            ...
            await fulfillController.requestUpdatePrices()
            await miOracle.fulfillRequest([{ tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(newBtcPrice), lastUpdate: 0 }], 0)
            await expect(orderBook.executeDecreaseOrder(order.account, orderIndex, user1.address), 3).to.not.be.revertedWith(
                'OrderBook: invalid price for execution'
            )
    ...
    it('Execute decrease order, non-existent', async () => {
        ...
        await expect(orderBook.executeDecreaseOrder(defaults.user.address, 1, user1.address)).to.be.revertedWith('OrderBook: non-existent order')
    })
    ...
    it('Execute decrease order, long', async () => {
        ...
        await increaseBlockTime(provider, 10)
        await fulfillController.requestUpdatePrices()
        await miOracle.fulfillRequest([{ tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(BTC_PRICE + 5050), lastUpdate: 0 }], 0)
    ...
    it('Execute decrease order, short, BTC', async () => {
        ...
        await increaseBlockTime(provider, 10)
        await fulfillController.requestUpdatePrices()
        await miOracle.fulfillRequest([{ tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(BTC_PRICE - 1500), lastUpdate: 0 }], 0)
    ...
    it('Cancel decrease order', async () => {
        ...
        await expect(orderBook.connect(defaults.user).cancelDecreaseOrder(1)).to.be.revertedWith('OrderBook: non-existent order')
