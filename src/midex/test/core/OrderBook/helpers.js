...
function getSwapFees(token, amount) {
    ...
    let feesPoints
    if ([dai.address, busd.address, musd.address].includes(token)) {
        feesPoints = 4
    } else {
        feesPoints = 30
    }
...
async function getMinOut(tokenDecimals, triggerRatio, path, amountIn) {
    ...
    let minOut = amountIn.mul(PRICE_PRECISION).div(triggerRatio).mul(tokenBPrecision).div(tokenAPrecision)
