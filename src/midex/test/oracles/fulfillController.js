...
describe('\n📌 ### Test fulfillController ###\n', function () {
    ...
    it('Test RequestOracleWithToken', async function () {
        ...
        await expect(fulfillController.connect(handler).requestOracleWithToken(data, AddressZero, btc.address, 1, false, [])).to.be.revertedWith(
            'address invalid'
        )
        // _transferETH = true
        await expect(fulfillController.connect(handler).requestOracleWithToken(data, user0.address, btc.address, 1, true, [])).to.be.revertedWith(
            'address invalid'
        )
        ...
        await expect(
            fulfillController.connect(handler).requestOracleWithToken(data, user0.address, btc.address, expandDecimals(3, 8), false, [])
    ...
    it('swap, refundTask', async function () {
        ...
        // 3. miOracle.request owner, reqId

        reqId = await miOracle.reqId()
        const request = await miOracle.requests(reqId)
        ...
        await miOracle.fulfillRequest(
            [
                ...
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(30000), lastUpdate: 0 },
            ],
            0 /* reqId */
        )
        ...
        await miOracle.fulfillRequest(
            [
                { tokenIndex: tokenIndexes.BUSD, price: toMiOraclePrice(1), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(29699), lastUpdate: 0 },
            ],
            lastReqId /* reqId */
        )
    ...
    it('Test requestOracleForLiquidate', async function () {
        ...
        await router.connect(user1).swap([busd.address, musd.address], expandDecimals(30000, 18), expandDecimals(29000, 18), user1.address)
        await miOracle.fulfillRequest(
            [
                { tokenIndex: tokenIndexes.BUSD, price: toMiOraclePrice(1), lastUpdate: 0 },
                { tokenIndex: tokenIndexes.BTC, price: toMiOraclePrice(60000), lastUpdate: 0 },
        ...
        await expect(
            positionManager.connect(liquidator).liquidatePosition(user0.address, bnb.address, bnb.address, true, liquidator.address)
