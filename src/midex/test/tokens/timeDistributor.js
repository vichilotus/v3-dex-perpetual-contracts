...
describe('TimeDistributor', function () {
    ...
    it('distribute', async () => {
        ...
        await expect(distributor.connect(user0).setDistribution([user1.address], [100], [token.address])).to.be.revertedWith(
        ...
        await expect(distributor.connect(user0).setTokensPerInterval(user1.address, 50)).to.be.revertedWith('TimeDistributor: forbidden')
        ...
        await expect(distributor.connect(user0).updateLastDistributionTime(user1.address)).to.be.revertedWith('TimeDistributor: forbidden')
