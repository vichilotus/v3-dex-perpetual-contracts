...
const { getJSONRequest, sleep } = require('../shared/utilities')
const { priceFeedApi, priceFeedSigners } = require('../../scripts/shared/config')

// check end with slash (/)
const api = priceFeedApi.endsWith('/') ? priceFeedApi : priceFeedApi + '/'
const tokenIndexes = {
    BTC: 0,
    ETH: 1,
    BNB: 2,
...
describe('\n📌 ### Test miOracle FulfillRequest ###\n', function () {
    ...
    it('fulfillRequest', async function () {
        ...
        for (let tokenIndex of Object.values(tokenIndexes)) {
            ...
            console.log(
                tokenIndex,
                await miOracle.getPriceFeed(tokenIndex),
                'round',
                +prices[0],
                'price',
                +prices[1],
...
async function deployMiOracle() {
    ...
    await miOracle.connect(deployer).setPriceFeedStore(list)

    // Set controller
    await miOracle.connect(deployer).setController(controller.address, true)

    // Set signer
    await miOracle.connect(deployer).setSigner(priceFeedSigners[0], true)
    await miOracle.connect(deployer).setSigner(priceFeedSigners[1], true)
    await miOracle.connect(deployer).setSigner(priceFeedSigners[2], true)
    await miOracle.connect(deployer).setSigner(priceFeedSigners[3], true)
