...
describe('ReferralStorage', function () {
    ...
    it('Sets new handler', async () => {
        await expect(referralStorage.connect(user0).setHandler(user1.address, true)).to.be.revertedWith(`GovernableUnauthorizedAccount("${user0.address}")`)

        expect(await referralStorage.isHandler(user0.address)).to.be.false
        await expect(referralStorage.connect(user1).setHandler(user0.address, true)).to.be.revertedWith(`GovernableUnauthorizedAccount("${user1.address}")`)
    ...
    it('setTier', async () => {
        await expect(referralStorage.connect(user0).setTier(0, 1000, 5000)).to.be.revertedWith(`GovernableUnauthorizedAccount("${user0.address}")`)

        let tier0 = await referralStorage.tiers(0)
        expect(tier0.totalRebate).eq(0)
        expect(tier0.discountShare).eq(0)

        await expect(referralStorage.setTier(0, 10001, 5000)).to.be.revertedWith('ReferralStorage: invalid totalRebate')

        await expect(referralStorage.setTier(0, 1000, 10001)).to.be.revertedWith('ReferralStorage: invalid discountShare')
    ...
    it('setReferrerTier', async () => {
        await expect(referralStorage.connect(user0).setReferrerTier(user0.address, 2)).to.be.revertedWith(`GovernableUnauthorizedAccount("${user0.address}")`)
    ...
    it('setReferrerDiscountShare', async () => {
        await expect(referralStorage.connect(user0).setReferrerDiscountShare(10001)).to.be.revertedWith('ReferralStorage: invalid discountShare')
    ...
    it('setTraderReferralCode', async () => {
        ...
        await expect(referralStorage.connect(user0).setTraderReferralCode(user1.address, code)).to.be.revertedWith('ReferralStorage: forbidden')
    ...
    it('Registers code', async () => {
        await expect(referralStorage.connect(user0).registerCode(HashZero)).to.be.revertedWith('ReferralStorage: invalid _code')
        ...
        await expect(referralStorage.connect(user0).registerCode(code)).to.be.revertedWith('ReferralStorage: code already exists')
    ...
    it('setCodeOwner', async () => {
        ...
        await expect(referralStorage.connect(user1).setCodeOwner(HashZero, user2.address)).to.be.revertedWith('ReferralStorage: invalid _code')
    ...
    it('govSetCodeOwner', async () => {
        ...
        await expect(referralStorage.connect(user1).registerCode(HashZero)).to.be.revertedWith('ReferralStorage: invalid _code')

        await expect(referralStorage.connect(user1).govSetCodeOwner(code, user2.address)).to.be.revertedWith(
            `GovernableUnauthorizedAccount("${user1.address}")`
        )
    ...
    it('timeLock.setTier', async () => {
        await referralStorage.transferGovernance(timeLock.address)

        await expect(referralStorage.setTier(1, 12, 20)).to.not.reverted

        await expect(timeLock.connect(user0).setTier(referralStorage.address, 1, 12, 20)).to.be.revertedWith('TimeLock: forbidden')
        ...
        await expect(timeLock.connect(user0).setTier(referralStorage.address, 2, 16, 40)).to.be.revertedWith(
            `GovernableUnauthorizedAccount("${timeLock.address}")`
        )

        tier = await referralStorage.tiers(2)
    ...
    it('timeLock.setReferrerTier', async () => {
        ...
        await expect(referralStorage.setReferrerTier(user1.address, 2)).to.not.reverted

        await expect(timeLock.connect(user0).setReferrerTier(referralStorage.address, user1.address, 2)).to.be.revertedWith('TimeLock: forbidden')

        await timeLock.setContractHandler(user0.address, true)

        expect(await referralStorage.referrerTiers(user1.address)).eq(2)
        await expect(timeLock.connect(user0).setReferrerTier(referralStorage.address, user1.address, 3)).to.be.revertedWith(
            `GovernableUnauthorizedAccount("${timeLock.address}")`
        )
        await timeLock.acceptGovernance(referralStorage.address)
        await timeLock.connect(user0).setReferrerTier(referralStorage.address, user1.address, 3)
        expect(await referralStorage.referrerTiers(user1.address)).eq(3)
    })
    ...
    it('timeLock.govSetCodeOwner', async () => {
        ...
        await expect(referralStorage.govSetCodeOwner(code, user1.address)).to.not.reverted

        await expect(timeLock.connect(user0).govSetCodeOwner(referralStorage.address, code, user1.address)).to.be.revertedWith('TimeLock: forbidden')

        await timeLock.setContractHandler(user0.address, true)

        expect(await referralStorage.codeOwners(code)).eq(user1.address)
        await expect(timeLock.connect(user0).govSetCodeOwner(referralStorage.address, code, user2.address)).to.be.revertedWith(
            `GovernableUnauthorizedAccount("${timeLock.address}")`
        )

        await expect(referralStorage.govSetCodeOwner(code, user2.address)).to.not.reverted
        expect(await referralStorage.codeOwners(code)).eq(user2.address)
    })
})
