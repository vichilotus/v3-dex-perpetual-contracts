...
async function main() {
    ...
    const distributorArr = [
        {
            name: 'FeeMilpDistributor',
            address: getContractAddress('feeMilpDistributor'),
            transferAmount: '0', // 20
    ...
    for (let i = 0; i < distributorArr.length; i++) {
        ...
        const distributor = await contractAt('RewardDistributor', distributorItem.address, signer)
        ...
        await sendTxn(distributor.updateLastDistributionTime(), `${distributorItem.rewardCName}.updateLastDistributionTime`)
        ...
        if (distributorItem.isRewardsPerInterval) {
            ...
            await sendTxn(distributor.setTokensPerInterval(rewardsPerInterval, { gasLimit: 5000000 }), 'rewardDistributor.setTokensPerInterval')
        }
    }
...
// async function getAjaxValues(signer) {
//   const rewardToken = await contractAt(
//     "Token",
