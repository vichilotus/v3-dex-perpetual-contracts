...
contract Reader is Governable {
    ...
    function setConfig(bool _hasMaxGlobalShortSizes) public onlyGovernor {
    ...
    function getMaxAmountIn(IVault _vault, address _tokenIn, address _tokenOut) public view returns (uint256) {
        ...
        uint256 maxMusdAmount = _vault.maxMusdAmounts(_tokenIn);

        if (maxMusdAmount != 0) {
            if (maxMusdAmount < _vault.musdAmounts(_tokenIn)) {
                return 0;
            }

            uint256 maxAmountIn = maxMusdAmount.sub(_vault.musdAmounts(_tokenIn));
            maxAmountIn = maxAmountIn.mul(10 ** tokenInDecimals).div(10 ** MUSD_DECIMALS);
    ...
    function getVaultTokenInfo(address _vault, address _weth, uint256 _musdAmount, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 3] = vault.getRedemptionAmount(token, _musdAmount, false);
    ...
    function getFullVaultTokenInfo(address _vault, address _weth, uint256 _musdAmount, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 6] = vault.maxMusdAmounts(token);
    ...
    function getVaultTokenInfoV2(address _vault, address _weth, uint256 _musdAmount, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _collateralTokens.length; i++) {
            {
                (
                    ...
                    ,
                    /* reserveAmount */ uint256 realizedPnl_,
                    bool hasRealizedProfit_,
                    uint256 lastIncreasedTime_
                ) = IVaultPositionController(_vaultPositionController).getPosition(_account, _collateralTokens[i], _indexTokens[i], _isLong[i]);
                ...
                amounts[i * POSITION_PROPS_LENGTH + 4] = hasRealizedProfit_ ? 1 : 0;
                amounts[i * POSITION_PROPS_LENGTH + 5] = realizedPnl_;
            ...
            if (averagePrice > 0) {
                (bool hasProfit, uint256 delta) = IVaultPositionController(_vaultPositionController).getDelta(
                    _indexTokens[i],
                    size,
                    averagePrice,
                    _isLong[i],
