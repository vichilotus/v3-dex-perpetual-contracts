...
contract VaultPriceFeed is IVaultPriceFeed, Governable {
    ...
    function setSpreadBasisPoints(address _token, uint256 _spreadBasisPoints) external override onlyGovernor {
        require(_spreadBasisPoints <= MAX_SPREAD_BASIS_POINTS, 'VaultPriceFeed: invalid _spreadBasisPoints');
        spreadBasisPoints[_token] = _spreadBasisPoints;
    }

    function setPriceSampleSpaceTime(uint256 _priceSampleSpaceTime) external override onlyGovernor {
        require(_priceSampleSpaceTime > 0, 'VaultPriceFeed: invalid _priceSampleSpaceTime');
        priceSampleSpaceTime = _priceSampleSpaceTime;
    }

    function setMaxStrictPriceDeviation(uint256 _maxStrictPriceDeviation) external override onlyGovernor {
        maxStrictPriceDeviation = _maxStrictPriceDeviation;
    }

    function setTokenConfig(address _token, address _priceFeed, uint256 _priceDecimals, bool _isStrictStable) external override onlyGovernor {
        priceFeeds[_token] = _priceFeed;
        priceDecimals[_token] = _priceDecimals;
        strictStableTokens[_token] = _isStrictStable;
    }

    function getPrice(address _token, bool _maximise, bool _validate) public view override returns (uint256) {
    ...
    function getPrimaryPrice(address _token, bool _maximise, bool _validate) public view override returns (uint256) {
