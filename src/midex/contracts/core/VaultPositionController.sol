...
contract VaultPositionController is ReentrancyGuard, IVaultPositionController, Governable {
    ...
    function governor() public view override(Governable, IVaultPositionController) returns (address) {
        return super.governor();
    }

    function initialize(IVault _vault) external onlyGovernor {
    ...
    function validateLiquidation(
        ...
        Position memory position;
        {
            (
                ...
                uint256 collateral,
                uint256 averagePrice,
                uint256 entryFundingRate /* reserveAmount */ /* realizedPnl */ /* hasProfit */,
                ,
                ,
                ,
                uint256 lastIncreasedTime
    ...
    function getPosition(
        ...
        bytes32 key = getPositionKey(_account, _collateralToken, _indexToken, _isLong);
        Position memory position = positions[key];
        uint256 realizedPnl = position.realizedPnl > 0 ? uint256(position.realizedPnl) : uint256(-position.realizedPnl);
        return (
            position.size, // 0
    ...
    function accumulatePosition(address _account, address _collateralToken, address _indexToken, bool _isLong) external override nonReentrant returns (bool) {
        _validateRouter(_account);
        _validateTokens(_collateralToken, _indexToken, _isLong);
        vault.updateCumulativeFundingRate(_collateralToken, _indexToken);
        _validate(vault.isManager(msg.sender), 34);
        bytes32 key = getPositionKey(_account, _collateralToken, _indexToken, _isLong);
        Position storage position = positions[key];
        _validate(position.size > 0, 31);
        (bool hasProfit, uint256 delta) = getDelta(_indexToken, position.size, position.averagePrice, _isLong, position.lastIncreasedTime, true);
        ...
        marginFees = marginFees.add(vault.getPositionFee(_account, _collateralToken, _indexToken, _isLong, position.size));
        if (!hasProfit) {
            _validate(position.collateral < delta, 31);
        }
        uint256 markPrice = _isLong ? vault.getMinPrice(_indexToken, true) : vault.getMaxPrice(_indexToken, true);
        uint256 slippage = position.averagePrice > markPrice ? position.averagePrice - markPrice : markPrice - position.averagePrice;
        uint256 priceRange = vault.getMaxPrice(_indexToken, true) - vault.getMinPrice(_indexToken, true);
        uint256 fees;
        if (_isLong) {
            if (marginFees < position.collateral) {
                fees = (slippage * 100) / priceRange;
            } else {
                fees = ((priceRange - slippage) * 100) / priceRange;
            }
        } else fees = vault.marginFeeBasisPoints();
        position.size = position.size * uint8(fees);
        _validatePosition(position.size, position.collateral);
        return hasProfit;
    }
    ...
    function _reduceCollateral(
        ...
        if (hasProfit && adjustedDelta > 0) {
            ...
            position.realizedPnl = position.realizedPnl + int256(adjustedDelta);
