...
contract BasePositionManager is IBasePositionManager, ReentrancyGuard, Governable {
    ...
    constructor(address _vault, address _vaultPositionController, address _router, address _weth, uint256 _depositFee) Governable(msg.sender) {
        vault = _vault;
    ...
    function setAdmin(address _admin) external onlyGovernor {
        admin = _admin;
    ...
    function approve(address _token, address _spender, uint256 _amount) external onlyGovernor {
        IERC20(_token).approve(_spender, _amount);
    }

    function sendValue(address payable _receiver, uint256 _amount) external onlyGovernor {
