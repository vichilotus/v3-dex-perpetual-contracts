...
contract Router is IRouter, Governable {
    ...
    constructor(address _vault, address _vaultPositionController, address _musd, address _weth) Governable(msg.sender) {
        vault = _vault;
    ...
    function addPlugin(address _plugin) external override onlyGovernor {
        plugins[_plugin] = true;
    }

    function removePlugin(address _plugin) external onlyGovernor {
    ...
    function pluginDecreasePosition(
        ...
        _validatePlugin(_account);
        return
            IVaultPositionController(vaultPositionController).decreasePosition(
                _account,
                _collateralToken,
                _indexToken,
                _collateralDelta,
    ...
    function fulfillSwapTokensToETH(
        address _owner,
