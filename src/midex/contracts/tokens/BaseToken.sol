...
contract BaseToken is IERC20, IBaseToken, Governable {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _initialSupply) Governable(msg.sender) {
        name = _name;
        symbol = _symbol;
        _mint(msg.sender, _initialSupply);
    }

    function setInfo(string memory _name, string memory _symbol) external onlyGovernor {
        name = _name;
        symbol = _symbol;
    }

    function setYieldTrackers(address[] memory _yieldTrackers) external onlyGovernor {
        yieldTrackers = _yieldTrackers;
    }

    function addAdmin(address _account) external onlyGovernor {
        admins[_account] = true;
    }

    function removeAdmin(address _account) external override onlyGovernor {
        admins[_account] = false;
    }

    // to help users who accidentally send their tokens to this contract
    function withdrawToken(address _token, address _account, uint256 _amount) external override onlyGovernor {
        IERC20(_token).safeTransfer(_account, _amount);
    }

    function setInPrivateTransferMode(bool _inPrivateTransferMode) external override onlyGovernor {
