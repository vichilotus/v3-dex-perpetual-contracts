...
// for example integration with miOracle
// ------------------------------
contract SimpleTrade {
  ...
  constructor(address _miOracle, address _weth) {
    ...
    tokenIndexList[5] = true; // allow DOGE
  }
  ...
  function openPosition(uint256 _tokenIndex, uint256 _amount) external {
    ...
    uint256 reqId = IMiOracle(miOracle).requestPrices(payload, 0); // with no expiration
  ...
  function _openPosition(bool _priceUpdate, uint256 _positionId) private {
    ...
    (, uint256 price, , ) = IMiOracle(miOracle).getLastPrice(position.tokenIndex);
