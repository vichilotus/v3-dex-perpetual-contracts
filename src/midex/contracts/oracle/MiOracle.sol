...
contract MiOracle is IPriceFeed, OwnableUpgradeable, PausableUpgradeable {
    // constants
    ...
    function setPriceFeedStore(address[] calldata _priceFeedStores) external onlyOwner {
    ...
    function getRequest(uint256 _reqId) external view returns (uint256, address, bytes memory, uint256, uint256, bool) {
        ...
        return (request.timestamp, request.owner, request.payload, request.status, request.expiration, paymentAvailable(request.owner));
    }
}
