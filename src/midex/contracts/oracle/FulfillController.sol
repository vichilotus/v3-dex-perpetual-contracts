...
interface IHandler {
    ...
    // RewardRouter
    function fulfillMintAndStakeMilp(address _account, address _token, uint256 _amount, uint256 _minMusd, uint256 _minMilp) external returns (uint256);
    ...
    function fulfillUnStakeAndRedeemMilpETH(address _account, uint256 _milpAmount, uint256 _minOut, address _receiver) external returns (uint256);
...
contract FulfillController is Ownable {
    ...
    constructor(address _miOracle, address _weth, uint256 _lastTaskId) {
    ...
    function requestOracle(bytes memory _data, address _account, bytes memory _revertHandler) external onlyHandler {
        ...
        IMiOracle(miOracle).requestPrices(payload, tasks[lastTaskId].expire);
    ...
    function requestUpdatePrices() external onlyController {
        ...
        IMiOracle(miOracle).requestPrices(payload, 0); // with no expiration
    }
    ...
    function miOracleCall(uint256 /* _reqId */, bool _priceUpdate, bytes memory _payload) external {
        // security callback
        require(msg.sender == miOracle, 'FulfillController: only miOracle callback');
        // decode payload
        ...
        if (!_priceUpdate && taskId != 0) {
            revertTask(taskId);
            emit FulfillTask(taskId, tasks[taskId].owner, false, 'miOracle: refundRequest', tasks[taskId].data);
            return;
        }
    ...
    function fulfillCall(address _to, bytes calldata _data) public onlyInternal {
        ...
        if (sig == 0x348621c8) {
            (address _owner, address[] memory _path, uint256 _amountIn, uint256 _minOut, address _receiver) = abi.decode(
                _data[4:],
        ...
        // - fulfillLiquidatePosition(address,address,address,bool,address)
            ...
            (address _account, address _collateralToken, address _indexToken, bool _isLong, address _feeReceiver) = abi.decode(
                _data[4:],
                (address, address, address, bool, address)
            );
        ...
        // - handlerAddLiquidity(address,address,address,uint256,uint256,uint256)
            ...
            (address _account, address _receiver, address _token, uint256 _amount, uint256 _minMusd, uint256 _minMilp) = abi.decode(
                _data[4:],
                (address, address, address, uint256, uint256, uint256)
            );
            IHandler(_to).handlerAddLiquidity(_account, _receiver, _token, _amount, _minMusd, _minMilp);
        }
        // - handlerRemoveLiquidity(address,address,address,uint256,uint256)
        else if (sig == 0x3b971a9e) {
            (address _account, address _receiver, address _tokenOut, uint256 _milpAmount, uint256 _minOut) = abi.decode(
                _data[4:],
                (address, address, address, uint256, uint256)
            );
            IHandler(_to).handlerRemoveLiquidity(_account, _receiver, _tokenOut, _milpAmount, _minOut);
        }
        // [RewardRouter]
        // - fulfillMintAndStakeMilp(address,address,uint256,uint256,uint256)
        else if (sig == 0x32f89f7b) {
            (address _account, address _token, uint256 _amount, uint256 _minMusd, uint256 _minMilp) = abi.decode(
                _data[4:],
                (address, address, uint256, uint256, uint256)
            );
            IHandler(_to).fulfillMintAndStakeMilp(_account, _token, _amount, _minMusd, _minMilp);
        }
        // - fulfillUnStakeAndRedeemMilp(address,address,uint256,uint256,address)
        else if (sig == 0x72b57547) {
            (address _account, address _tokenOut, uint256 _milpAmount, uint256 _minOut, address _receiver) = abi.decode(
                _data[4:],
                (address, address, uint256, uint256, address)
            );
            IHandler(_to).fulfillUnStakeAndRedeemMilp(_account, _tokenOut, _milpAmount, _minOut, _receiver);
        }
        // - fulfillUnStakeAndRedeemMilpETH(address,uint256,uint256,address)
        else if (sig == 0xc1b5dab8) {
            (address _account, uint256 _milpAmount, uint256 _minOut, address _receiver) = abi.decode(_data[4:], (address, uint256, uint256, address));
            IHandler(_to).fulfillUnStakeAndRedeemMilpETH(_account, _milpAmount, _minOut, _receiver);
        }
