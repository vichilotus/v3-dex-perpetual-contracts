...
contract RewardTracker is IERC20, ReentrancyGuard, IRewardTracker, Governable {
    ...
    constructor(string memory _name, string memory _symbol) Governable(msg.sender) {
        name = _name;
        symbol = _symbol;
    }

    function initialize(address[] memory _depositTokens, address _distributor) external onlyGovernor {
    ...
    function setDepositToken(address _depositToken, bool _isDepositToken) external onlyGovernor {
        isDepositToken[_depositToken] = _isDepositToken;
    }

    function setInPrivateTransferMode(bool _inPrivateTransferMode) external onlyGovernor {
        inPrivateTransferMode = _inPrivateTransferMode;
    }

    function setInPrivateStakingMode(bool _inPrivateStakingMode) external onlyGovernor {
        inPrivateStakingMode = _inPrivateStakingMode;
    }

    function setInPrivateClaimingMode(bool _inPrivateClaimingMode) external onlyGovernor {
