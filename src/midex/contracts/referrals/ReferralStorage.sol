...
contract ReferralStorage is Governable, IReferralStorage {
    ...
    function setTier(uint256 _tierId, uint256 _totalRebate, uint256 _discountShare) external override onlyGovernor {
    ...
    function setReferrerTier(address _referrer, uint256 _tierId) external override onlyGovernor {
    ...
    function govSetCodeOwner(bytes32 _code, address _newAccount) external override onlyGovernor {
        require(_code != bytes32(0), 'ReferralStorage: invalid _code');
